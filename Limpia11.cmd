...
Title Limpia11 - Version %fileSize%
echo.
echo Limpia 11 v%fileSize%
echo ----------------
echo.
...
FOR /F %%g IN ('"%es%" -get-result-count') do (
	title Limpia11 v%fileSize% - Archivos: %%g
...
for /f "usebackq tokens=*" %%f in ("%~dp0list_folders.dat") do (
    echo      - %%f
        FOR /f "tokens=*" %%A IN ('%es% -w -sort path-ascending folder:%%f %param_drive%') DO (
			rmdir /S /Q "%%A" >nul 2>&1
